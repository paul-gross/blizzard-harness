...
As of **P7 (wave 1)** the standing **e2e smoke test** (`blizzard:e2e` — `mise run e2e`) covers **three** full-stack scenarios over the whole `build → review → deliver` default shape: (1) the happy path travels build → review (scripted PASS) → deliver → landed; (2) the review-fail cycle fails review once, carries the findings asset + prompt_addendum back into build's re-entry envelope, and lands on the second pass; (3) retries exhausted — two verdict-less exits derive `needs_human`, whose surfaced takeover command, run verbatim, actually resumes the parked mock session. Each is asserted at both ends (git truth on bare `main`; hub facts derive the terminal status). The **heartbeat/stall detection** (REAP staleness + `POST /api/heartbeat` + `blizzard runner heartbeat`), the **store-and-forward outbound buffer** (FIFO drain, seq-idempotent `POST /api/events` against the hub high-water mark), and the **epoch fence** (lease-report keystone; zombie/stale-completion rejection) also landed this wave and are covered by the component tier. They join the P5 Python-QA/frontend/wheel rows and the P4 `blizzard-mock` rows as real. What remains ahead of the code is the **P7** upper tier: the *running-daemon* service tier and browser e2e exercised from outside (Playwright), and the kill-9 crash sweep, which still need the crash-point registry and the invariant checker (heartbeat/stall detection, the piece they built on, now exists).
