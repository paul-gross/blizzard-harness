...
As of **P7 (wave 2)** the standing **e2e smoke test** (`blizzard:e2e` — `mise run e2e`) covers **five** full-stack scenarios over the whole `build → review → deliver` default shape and its human-loop variants: (1) the happy path travels build → review (scripted PASS) → deliver → landed; (2) the review-fail cycle fails review once, carries the findings asset + prompt_addendum back into build's re-entry envelope, and lands on the second pass; (3) retries exhausted — two verdict-less exits derive `needs_human`, whose surfaced takeover command, run verbatim, actually resumes the parked mock session; (4) **ask/answer** — a build worker asks a question and exits, the chunk derives `waiting_on_human` with the reap clock stopped (extra ticks consume no retry), `blizzard hub answer` resumes the same dormant session (the mock's persisted session state proves it read the answer) and the chunk lands; (5) **human gate** — a graph with an `approve-gate` ahead of deliver parks an open Decision carrying the build artifact, `blizzard hub decisions` lists it, `blizzard hub decide` approves it, and the chunk delivers and lands. Each is asserted at both ends (git truth on bare `main`; hub facts derive the terminal status). The wave-1 **heartbeat/stall detection** (REAP staleness + `POST /api/heartbeat` + `blizzard runner heartbeat`), **store-and-forward outbound buffer** (FIFO drain, seq-idempotent `POST /api/events` against the hub high-water mark), and **epoch fence** (lease-report keystone; zombie/stale-completion rejection) are covered by the component tier; the wave-2 **human loop** — ask/answer park→resume, graph and runner-config gate decisions, first-write-wins resolution, and requeue supersession, plus the `blizzard runner ask` / `blizzard hub answer` / `blizzard hub decisions` / `blizzard hub decide` / `blizzard hub requeue` CLI verbs — is covered by the component tier and, end to end, by e2e scenarios (4) and (5). They join the P5 Python-QA/frontend/wheel rows and the P4 `blizzard-mock` rows as real. What remains ahead of the code is the **P7** upper tier: the *running-daemon* service tier and browser e2e exercised from outside (Playwright), and the kill-9 crash sweep, which still need the crash-point registry and the invariant checker (heartbeat/stall detection, the piece they built on, now exists).
...
| blizzard:crash-sweep | The kill-9 sweep: a pytest-driven runner iterating the crash-point registry, running the daemons as real subprocesses and asserting the invariant checker after each armed crash ([../architecture/crash-correctness.md](../architecture/crash-correctness.md)). **Gap (P7)** — heartbeat/stall detection now exists (P7 wave 1: REAP staleness + the runner-store heartbeat table), so what remains is the crash-point registry and the invariant checker, over the steppable loop and injected clock (P6). |
